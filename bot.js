const Discord = require('discord.js');  //cяz#0853
const client = new Discord.Client();  //cяz#0853
const moment = require('moment');
const zalgo = require('zalgolize');  //cяz#0853
const math = require('math-expression-evaluator');   //cяz#0853
const figlet = require('figlet');   //cяz#0853
const fs = require('fs');  //cяz#0853
const ms = require('ms');  //cяz#0853
const prefix = '-'

client.on('message', msg => {  //cяz#0853
    if (msg.content === 'باك') {  //cяz#0853
      msg.reply('** :wink: وِلِـكُمِـ ﻧَوِرُتْ   :sparkling_heart:**');  //cяz#0853
    }
  });

client.on('message', msg => {  //cяz#0853
    if (msg.content === 'crz') {  //cяz#0853
      msg.reply('**مشغول لا تتكلم**');  //cяz#0853
    }
  });


client.on('message', msg => {  //cяz#0853
    if (msg.content === 'احبك') {  //cяz#0853
      msg.reply('**لاتحتك يا اخوي لو سمحت**');  //cяz#0853
    }
  });
  ...
  client.on('message', msg => {  //cяz#0853
    if (msg.content === 'احبك') {  //cяz#0853
      msg.reply('**حبك برص يا سافل**');  //cяz#0853
    }
  });
   ...
   client.on('message', msg => {  //cяz#0853
    if (msg.content === 'هلا') {  //cяz#0853
      msg.reply('**هلا بيك :heart: **');  //cяz#0853
    }
  });
  ...
  client.on("message", message => {  //cяz#0853
    if(message.content.startsWith(prefix + 'v2min')) {  //cяz#0853
     let args = message.content.split(" ").slice(1);  //cяz#0853
       var nam = args.join(' ');
...
client.on("message", message => {    //cяz#0853
    if(message.content.startsWith(prefix + "emoji")) {   //cяz#0853
        if(message.author.bot) return;  //cяz#0853
        var emojiid =  message.content.split(" ").slice(1).join(" ")   //cяz#0853
        console.log(emojiid) 
        if(emojiid.length < "18" || emojiid.length > "18" || isNaN(emojiid)) return  message.channel.send(`- Usage  //cяz#0853
${prefix}emoji <EmojiID>`); 
        else    
        message.channel.send("This is the emoji that you requested:-",  //cяz#0853
          { 
            files: [`https://cdn.discordapp.com/emojis/${emojiid}.png`]  //cяz#0853
          }) 
        }  
})   
 ...
 client.on("message", message => {  //cяz#0853
  if (message.channel.type === "dm") {    //cяz#0853

      message.channel.startTyping();    //cяz#0853
      setTimeout(() => {    //cяz#0853
        message.channel.stopTyping();    //cяz#0853
      }, Math.random() * (1 - 3) + 1 * 1000);  //cяz#0853
   
  }  
});  

const clans = {};  //cяz#0853
const system = {};  //cяz#0853
const level = {};  //cяz#0853
 
 
client.on('message',async message => {  //cяz#0853
  if(message.author.bot) return;  //cяz#0853
  if(message.channel.type === 'dm') return;  //cяz#0853
 
  let args = message.content.split(' ');  //cяz#0853
  let random = Math.floor(Math.random() * 5) + 2;  //cяz#0853
  let author = message.author;
 
  let xpLeft;  //cяz#0853
  let nameClan;  //cяz#0853
  let membersClan = [];  //cяz#0853
  let levelClan = 0;  //cяz#0853
  if(!system[author.id]) system[author.id] = {clan: 'None',joinedAt: new Date().toLocaleString() ,clanLevel: 0};  //cяz#0853
 
  if(!level[author.id]) level[author.id] = {level: 1, xp: 1};  //cяz#0853
 
 
  level[author.id].xp += (+random);  //cяz#0853
  if(level[author.id].xp >= 300) {  //cяz#0853
    if(level[author.id].xp > 300) xpLeft = level[author.id].xp - 300;  //cяz#0853
    level[author.id] = {  //WESO#0001
      level: level[author.id].level + 1,  //cяz#0853
      xp: xpLeft  //cяz#0853
    };
 
  }
  if(message.content.startsWith(prefix + "clan")) {  //cяz#0853
    if(message.content.split(' ')[0] !== `${prefix}clan`) return;  //cяz#0853
 
    if(!args[1] || args[1] && args[1] === 'info') {  //cяz#0853
  let embed = new Discord.RichEmbed()
  .setAuthor('الكلانات', message.author.avatarURL)  //cяz#0853
...
client.login(process.env.633284396144787476);
